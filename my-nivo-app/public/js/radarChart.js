function RadarChart(id, data, options, names,contestDuration) {
    ...
    axisGrid.selectAll(".axisLabel")
        ...
        .attr("fill", "black")
        .text(function (d, i) { return (parseFloat(maxValue * d / cfg.levels)*100).toFixed(2).toString() + "%"; });
...
async function Plot() {
    ...
    async function fetchData(handles, contest) {
        ...
        try {
            const userDataPromises = handleList.map(async (user) => {
                ...
                while (retryCount < 3) { // Retry up to 3 times
                    try {
                        ...
                        if (!response.ok) {
                            throw new Error(`Failed to fetch data for ${user}. Status code: ${response.status}`);
                        }
            ...
            while (contestRetryCount < 3) { // Retry up to 3 times for contest data
                try {
                    ...
                    if (!contestResponse.ok) {
                        throw new Error(`Failed to fetch contest data. Status code: ${contestResponse.status}`);
                    }
